...
import { useNewWorkflow } from '../../_hooks/useNewWorkflow.hook';
...
export default function NewWorkflow() {
	...
	const { isLoading, newWorkflow, setNewWorkflow, handleCreateWorkflow, errorMessage, onCloseModal } =
		useNewWorkflow();

	if (!activeModals.includes(MODAL_IDS.NEW_WORKFLOW)) return null;
	return (
		<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-md">
			...
			>
				...
				<div className="mb-8 text-center">
					<h2 className="gradient-primary-light bg-clip-text text-3xl font-bold text-transparent">Mission</h2>
				</div>

				<form onSubmit={handleCreateWorkflow} className="flex flex-col gap-2.5">
					<div className="group relative">
						...
						<div className="relative flex items-center rounded-xl border border-border bg-gray-800 p-4 transition-colors group-focus-within:border-primary">
							<FiFile className="mr-3 text-xl text-muted" />
							<input
								value={newWorkflow}
								onChange={(event) => setNewWorkflow(event.target.value)}
								max={15}
								autoFocus
