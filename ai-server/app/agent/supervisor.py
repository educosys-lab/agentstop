...
async def run_multi_agent(args: AgentArgs) -> DefaultReturnType[dict[str, Any] | Any]:
    try:
        ...
        if args.memory_id:
            ...
            dynamic_agent = create_react_agent(model=model, tools=tools, response_format=structured_format)

        analytics_agent = await get_analytics_agent()

        supervisor = create_supervisor(
            model=ChatOpenAI(model="openai:gpt-4o"),
            agents=[dynamic_agent, analytics_agent],
            prompt=(
                "You manage a default dynamic agent and a analytics agent. Assign work to them. Use the analytics agent only to get analytics data and analyze the data."
            ),
            add_handoff_back_messages=True,
            output_mode="full_history",
        ).compile()

        response = await supervisor.ainvoke({"messages": args.messages})
...
async def get_analytics_agent():
    tools_config: dict[Any, Any] = {
        "analytics-tool": {
            "command": "python",
            "args": ["app/tools/whatsapp/analytics.py"],
            "transport": "stdio",
            "env": {},
        }
    }
    ...
    mcp_client = MultiServerMCPClient(tools_config)
    tools: Any = await mcp_client.get_tools()

    return create_react_agent(
        name="analytics_agent",
        model="openai:gpt-4o",
        tools=[tools],
        prompt="You are a analytics agent. You are given a user id and a campaign name. You need to get the analytics data for the campaign and return the data.",
        # response_format=structured_format,
    )
